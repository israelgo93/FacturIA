...
const styles = StyleSheet.create({
	...
	proveedorBox: { backgroundColor: '#e8f5e9', padding: 5, marginBottom: 10, borderRadius: 2 },
	footer: { position: 'absolute', bottom: 20, left: 30, right: 30, textAlign: 'center', fontSize: 6, color: '#999' },
});

export default function RIDELiquidacionCompra({ comprobante }) {
			...
			<Page size="A4" style={styles.page}>
				...
				<View style={styles.header}>
					...
					<View style={styles.headerRight}>
						<Text style={styles.title}>LIQUIDACIÓN DE COMPRA</Text>
				...
				<View style={styles.proveedorBox}>
					...
					<View style={styles.row}>
						...
						<Text style={styles.value}>{comp.razon_social_proveedor || ''}</Text>
					</View>
					<View style={styles.row}>
						<Text style={styles.label}>Identificación: </Text>
						<Text style={styles.value}>{comp.identificacion_proveedor || ''}</Text>
					</View>
					{comp.direccion_proveedor && (
						<View style={styles.row}>
							<Text style={styles.label}>Dirección: </Text>
							<Text style={styles.value}>{comp.direccion_proveedor}</Text>
						</View>
					)}
				</View>
				...
				<View style={styles.totalesSection}>
					...
					<View style={styles.totalRow}>
						...
						<Text style={styles.totalValue}>{Number(comp.total_descuento || 0).toFixed(2)}</Text>
					</View>
					<View style={styles.totalRow}>
