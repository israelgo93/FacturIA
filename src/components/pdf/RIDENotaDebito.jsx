...
export default function RIDENotaDebito({ comprobante }) {
	const comp = comprobante;
	const motivos = comp.detalles || []; // Los motivos vienen como detalles

	// Calcular total de valores
	const totalValor = motivos.reduce((sum, m) => sum + Number(m.valor || m.precio_total_sin_impuesto || 0), 0);
			...
			<Page size="A4" style={styles.page}>
				...
				<View style={styles.section}>
					...
					<View style={styles.tableHeader}>
						<Text style={styles.colRazon}>Razón</Text>
						<Text style={styles.colValor}>Valor</Text>
					</View>
					{motivos.map((mot, i) => (
						<View key={i} style={styles.tableRow}>
							<Text style={styles.colRazon}>{mot.descripcion || mot.razon || ''}</Text>
							<Text style={styles.colValor}>{Number(mot.valor || mot.precio_total_sin_impuesto || 0).toFixed(2)}</Text>
						</View>
					))}
				</View>
				...
				<View style={styles.totalesSection}>
					<View style={styles.totalRow}>
						...
						<Text style={styles.totalValue}>{Number(comp.subtotal_sin_impuestos || totalValor).toFixed(2)}</Text>
					</View>
					<View style={styles.totalRow}>
